...
namespace util {
    ...
    {
        return pids_list.find(pid) != pids_list.end();
    }
...
{
    bool found = false;
    const bool check_time = (hh_args.ptimes != TIME_UNDEFINED);
    // filter by the time
...
void HHScanner::performVTLookups(HHScanReport& report)
{
    if (hh_args.vt_api_key.empty()) return;
    if (report.suspicious.empty()) return;

    for (const DWORD pid : report.suspicious) {
        auto nameIt = report.pidToName.find(pid);
        if (nameIt != report.pidToName.end() && !hh_args.vt_ignore_list.empty()) {
            std::wstring lowerName = nameIt->second;
            std::transform(lowerName.begin(), lowerName.end(), lowerName.begin(), towlower);
            bool skip = false;
            for (const auto& ignored : hh_args.vt_ignore_list) {
                std::wstring lowerIgnored = ignored;
                std::transform(lowerIgnored.begin(), lowerIgnored.end(), lowerIgnored.begin(), towlower);
                if (lowerName.find(lowerIgnored) != std::wstring::npos) {
                    skip = true;
                    break;
                }
            }
        ...
        std::string proc_dir = files_util::join_path(this->outDir, "process_" + std::to_string(pid));

        WIN32_FIND_DATAA findData;
        std::string searchPath = proc_dir + "\\*.*";
        ...
        do {
            ...
            if (fname.find(".json") != std::string::npos) continue;

            std::string file_path = files_util::join_path(proc_dir, fname);
            std::string hash = vt::sha256_file(file_path);
            if (hash.empty()) continue;

            vt::VTResult vt_result = vt::lookup_hash(hh_args.vt_api_key, hash);

            std::string vt_report_path = file_path + ".vt.json";
            vt::write_vt_report(vt_report_path, vt_result);

            if (first_lookup && vt_result.found) {
                report.pidToVT[pid] = vt_result;
                first_lookup = false;
            }

            if (!hh_args.quiet && vt_result.found) {
                const std::lock_guard<std::mutex> stdOutLock(g_stdOutMutex);
                int color = (vt_result.positives > 0) ? RED : LIME;
                WORD old_color = set_color(color);
                std::cout << "   [VT] " << fname << " : " << vt_result.scoreStr();
                if (!vt_result.threat_label.empty()) {
                    std::cout << " (" << vt_result.threat_label << ")";
                }
                set_color(old_color);
                std::cout << std::endl;
            }
            else if (!hh_args.quiet && vt_result.error) {
                const std::lock_guard<std::mutex> stdOutLock(g_stdOutMutex);
                std::cerr << "   [VT] " << fname << " : error - " << vt_result.error_msg << std::endl;
            }
...
std::unique_ptr<HHScanReport> HHScanner::scan()
{
    const time_t scan_start = time(NULL); //start time of the current scan
    pesieve::t_params &pesieve_args = this->hh_args.pesieve_args;
    initOutDir(scan_start, pesieve_args);

    auto my_report = std::make_unique<HHScanReport>(GetTickCount(), scan_start);
    scanProcesses(*my_report);

    performVTLookups(*my_report);

    my_report->setEndTick(GetTickCount(), time(NULL));
    return my_report;
}

bool HHScanner::writeToLog(const HHScanReport& hh_report)
{
    if (!hh_args.log) {
        return false;
    }
