...
#define PARAM_VT "vt"
#define PARAM_VT_IGNORE "vtignore"
...
public:
        ...
        : Params(version)
    {
        ...
        //PARAM_VT
        this->addParam(new StringParam(PARAM_VT, false));
        this->setInfo(PARAM_VT, "Enable VirusTotal lookup for suspicious dumps. Provide your API key.");

        {
            std::stringstream ss1;
            ss1 << "Skip VirusTotal lookups for processes with given names.";
            std::stringstream ss2;
            ss2 << INFO_SPACER << "Example: AnyDesk.exe" << PARAM_LIST_SEPARATOR << "TeamViewer.exe";
            this->addParam(new StringListParam(PARAM_VT_IGNORE, false, PARAM_LIST_SEPARATOR));
            this->setInfo(PARAM_VT_IGNORE, ss1.str(), ss2.str());
        }
        ...
        this->addParamToGroup(PARAM_VT, str_group);
        this->addParamToGroup(PARAM_VT_IGNORE, str_group);
    ...
    void fillStruct(t_hh_params& ps)
    {
        ...
        copyVal<StringParam>(PARAM_VT, ps.vt_api_key);

        StringListParam* vtIgnoreParam = dynamic_cast<StringListParam*>(this->getParam(PARAM_VT_IGNORE));
        if (vtIgnoreParam && vtIgnoreParam->isSet()) {
            std::set<std::string> vt_ignore_names;
            vtIgnoreParam->stripToElements(vt_ignore_names);
            for (const auto& name : vt_ignore_names) {
                ps.vt_ignore_list.insert(to_wstring(name));
            }
        }
